...
def run():
    ...
    print("Zooming too much breaks some \"special\" connections, due to time constraints I wasn't able to find a better solution to that problem!")
...
class StationMapPlotter:
    SC_MIN_FONT_SIZE = 6
    SC_MAX_FONT_SIZE = 7
    SC_JOIN_MIN_FONT_SIZE = 4
    SC_JOIN_MAX_FONT_SIZE = 5
    SD_MIN_FONT_SIZE = 5
    SD_MAX_FONT_SIZE = 8
    ...
    def draw_labels(self):
        for node_key, node_attributes in self.__graph.nodes(data=True):
           ...
           #avoid drawing the separate joins but still draw the normal code
            if SHOW_CODES:
    ...
    def __draw_edge_label(self, station_line: "StationLine", connection: "Connection", distance: float):
        if connection.connection_style == "special":
            return
